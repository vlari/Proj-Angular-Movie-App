...
export class NewsComponent implements OnInit {
  ...
  ngOnInit() {
    let resolvedNews: NewsResponse | HttpNewsError = this.route.snapshot.data[
      "resolvedNews"
    ];
    ...
    if (
      resolvedNews instanceof HttpNewsError ||
      resolvedHeadlines instanceof HttpNewsError
    ) {
    } else {
  ...
  filterNews() {
    this.newsDataService
      .getFilteredNews(this.filterText)
  ...
  loadNext() {
    ...
    this.loading = true;
    this.newsDataService
      .getNews(this.pageSize, this.pageToLoadNext)
